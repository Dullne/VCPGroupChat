...
- `GROUPCHAT_ROLE_STUDIO_MODELS`: optional comma-separated model list for role studio draft generation.
...
The repository tracks source, tests, scripts, Docker files, and example configuration only. Runtime data and secrets are ignored:
...
Do not commit real API keys, tokens, runtime SQLite databases, local caches, or machine-specific coordination state.
